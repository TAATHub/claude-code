...
| `genre: <値>` あり、かつ `wiki/<値>/` が存在しない（typo / 未 init） | 通常の triage 対象に戻す（A-0-2 で再判定。frontmatter の値も新ジャンル候補スラッグの 1 つとして採用候補に含める） |
...
3. **確信度を 3 段階で付与**（**ユーザー確認は行わず、すべて自動振り分け**する。確信度は完了レポートでの透明性確保のためにのみ記録）:

| 確信度 | 条件 | 自動振り分け方針 |
|---|---|---|
| 高 | 単一既存ジャンルに明確にマッチ | 提案ジャンル（既存スラッグ）をそのまま採用 |
| 中 | 複数候補に該当しうる | 最有力候補（既存スラッグ）を採用 |
| 低 / 該当なし | 既存ジャンルどれにも収まらない | 新ジャンル候補の **第 1 候補スラッグ** で `init <genre>` を自動実行して採用 |

新ジャンル候補スラッグは A-0-3 の自動振り分けで `init` の引数として使うため、必ず **小文字英数+ハイフン** の有効スラッグを 1 件以上生成すること（複数生成した場合は先頭を採用）。スラッグと対応する日本語名をペアで記録しておく（例: `data-science` (データサイエンス)）。

各ファイルについて内部記録: ファイルパス / 採用ジャンル（既存スラッグ or 新規スラッグ）/ 確信度 / 判定根拠（1 行）。この内部記録は A-0-3 の自動振り分けで使用し、完了レポートにも全件分を必ず転記する。

### A-0-3. 自動振り分け

**ユーザー確認は行わない**。A-0-2 で記録した採用ジャンル（既存スラッグ or 新規スラッグ）にそのまま従って A-0-4 へ直行する。確信度に関わらず `AskUserQuestion` を呼ばないことが原則。

判定済み一覧は **テキストで進捗ログとして提示**（処理を止めずに表示するだけ）してから A-0-4 へ進む:
...
3 | db-tuning.md | rdb | 中 | performance 系の新ジャンルもあり得るが既存 rdb と最も親和
4 | ds-talk.md | data-science (新規作成) | 低 | 既存ジャンルに該当なし、新ジャンルとして init
5 | misc.md | misc (新規作成) | 低 | 主題不明だが新ジャンル misc として保管
```

この一覧（採用ジャンル・確信度・判定根拠）は完了レポートにそのまま転記するため、内部記録として保持する。確信度「中」「低」のものは完了レポートの「要レビュー事項」にも併せて列挙し、後から人間が見直しやすいようにする。

> ユーザーが対話的に介入したい場合は、ingest 完了後に `wiki/<genre>/` 構造を見て手動で `mv` + `genre` フィールド書き換え、必要なら recompile を行う想定。triage 中に止めるフックは設けない。
...
- 判定方針は A-0-2 と同じ 3 段階の確信度評価を用い、**ユーザー確認は行わず A-0-3 と同じ自動振り分け** で進める（中・低も自動採用、低/該当なしは新ジャンル候補の第 1 スラッグで自動 `init`）
- 採用ジャンル・確信度・判定根拠は完了レポートに必ず記録する。確信度「中」「低」は「要レビュー事項」にも併記する
...
- 振り分け結果の内訳: 既存ジャンルへ自動振り分け / 新ジャンル自動作成 + 移動 / 異常状態で残置（`type: source-summary` 既設のため） / fast path 直行
