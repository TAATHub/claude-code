...
description: YouTube動画のリンクから文字起こし(トランスクリプト/スクリプト)を取得し、要約・詳細解説・全文をMarkdownファイルに書き出すスキル。yt-dlpで字幕を取得し日本語優先で整形し、必要に応じてWeb検索で文脈を補強する。「YouTubeの文字起こし」「動画のトランスクリプト」「この動画をMarkdownに」「youtube-transcript」などのリクエストで使用。
...
- **不確かな固有名詞・専門用語・製品名・数値の確認**: 自動字幕の聞き起こし誤りが疑われる箇所を正しい表記・定義に裏取りする。
- **前提知識の補足**: 動画が背景知識を前提にしていて、要約・解説の理解に文脈追加が有益なとき。
- **言及された外部リソースの特定**: 動画内で触れられたツール・論文・URL等の正式名称や所在を特定するとき。

補強時の原則:

- **トランスクリプト優先**: 検索結果がトランスクリプトと矛盾する場合は、トランスクリプトを一次情報として扱い、相違点を注記する（字幕誤りの可能性も併記）。
- **出所を明示**: 外部情報は本文中で「（補足: …）」のように区別し、`## 補足・参考` セクションに**出典URL付き**でまとめる。
- **全文は不可侵**: `## 全文トランスクリプト` には外部情報を混ぜない。
- **過剰検索をしない**: トランスクリプトだけで十分自己完結する動画では検索しない。補強は質の向上に寄与する範囲にとどめる。
...
- ユーザーに、作成したファイルへの**クリック可能なMarkdownリンク**と、取得した字幕言語・語数・主な内容の要点を報告する。
  Web検索で補強した場合は、その旨と補った観点（例: 用語の裏取り、背景補足）も一言添える。
